...
#define IS_INTEGRAL(T) (is_integral_v<T>)
#define IS_UNSIGNED(T) (is_unsigned_v<T>)
...
int main(){
	...
	int n, qn;
	cin >> n >> qn;
	vector<int> gold(n), silver(n);
	copy_n(istream_iterator<int>(cin), n, gold.begin());
	copy_n(istream_iterator<int>(cin), n, silver.begin());
	int ng = accumulate(gold.begin(), gold.end(), 0);
	int ns = accumulate(silver.begin(), silver.end(), 0);
	vector<int> prefg(n + 1), prefs(n + 1);
	for(auto i = 0; i < n; ++ i){
		prefg[i + 1] = prefg[i] + gold[i];
		prefs[i + 1] = prefs[i] + silver[i];
	}
	combinatorics<modular> C(ns);
	vector<modular> pref(ns + 2);
	for(auto i = 0; i <= ns; ++ i){
		pref[i + 1] = pref[i] + C.C(ns, i);
	}
	modular coef = modular(2).power(-ns);
	for(auto qi = 0; qi < qn; ++ qi){
		int ql, qr;
		cin >> ql >> qr, -- ql;
		int g0 = prefg[qr] - prefg[ql];
		int s0 = prefs[qr] - prefs[ql];
		int g1 = ng - g0;
		int s1 = ns - s0;
		cout << coef * pref[clamp(g0 + s0 - g1, 0, ns + 1)] << " ";
	}
	cout << "\n";
	return 0;
}
